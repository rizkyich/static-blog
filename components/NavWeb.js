...
export default function NavBar ({getNavOpen}) {
  ...
  useEffect(() => {
    if (navOpen) getNavOpen(true)
    else getNavOpen(false)
  }, [navOpen])
    ...
    <nav id="nav-web" className="relative py-3 px-6 md:px-16 xl:px-32 2xl:px-60 shadow-md flex justify-between">
      ...
      {/* </div> */}
      {
        ...
        <div style={{backgroundColor: 'rgba(0,0,0,.2)'}} className="w-full absolute top-12 left-0 md:hidden  h-screen overflow-hidden">
            <div className="nav-open absolute top-0 left-0 w-full rounded-b-3xl z-50 h-72 transition-all duration-150">
              <ul className="flex flex-col items-center w-full z-50 h-full py-8 justify-between">
                <li className="my-2">
                  <a href="https://higo.id/about-us">About Us</a>
                </li>
                <li className="my-2">
          ...
          <div onClick={() => setNavOpen(false)} className="w-full absolute top-0 h-screen bg-black opacity-40"></div>
        </div>
      }
    </nav>
  )
